...
public class AgentCoordinator
{
    ...
    public async Task<List<AgentProgress>> RunFullAnalysisAsync(Guid problemId, Action<List<AgentProgress>>? onProgress = null, CancellationToken cancellationToken = default)
    {
        var steps = new List<AgentProgress>
        {
            ...
            new() { Step = "Rakip Analizi", Status = AgentStepStatus.Pending }
        };
        ...
        try
        {
            await _analyzeCompetitorsHandler.HandleAsync(new AnalyzeCompetitorsCommand
            {
                ProblemId = problemId
            }, cancellationToken);
            ReportProgress(7, AgentStepStatus.Completed);
        }
        catch (Exception ex)
        {
